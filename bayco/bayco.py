...
def df_to_BFs(gene_list: List[str], all_genes_data: pd.DataFrame, number_of_pairs: int, mode: str = 'positive') -> pd.DataFrame:
...
def df_to_BFs_all_results(gene_list: List[str], all_genes_data: pd.DataFrame, number_of_pairs: int, mode: str = 'positive') -> Dict[str, pd.DataFrame]: 
...
def combine_networks(networks: List[pd.DataFrame]) -> pd.DataFrame:
